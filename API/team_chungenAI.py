...
ITEM_USE_SCORE = [0, 100, 250, 250, 300, 100, 100, 300]
ITEM_KEEP_SCORE = [0, 0, 0, 0, 0, 0, 0, 0]
...
class SelfState(object):
    def __init__(self, life, pos, vel, voltage, item_id,\
        ...
        self.attack_distance = attack_distance.copy()
        self.item_used = item_used.copy()
    ...
    def child_state(self, step):
        ...
        next_attack_distance = self.attack_distance.copy()
        next_item_used = self.item_used.copy()
        ...
        if step == AI_DIR_LEFT:
            ...
            for pos in self.env.other_player_pos:
                d = self.env.helper.get_distance(next_pos, pos)
                if d < self.env.self_attack_radius:
                    next_attack_distance.append(d)

        elif step == AI_DIR_USE_ITEM:
            if next_item_id == 0:
                return None
            next_item_used.append(next_item_id)
            if next_item_id == 1:
                pass
            elif next_item_id == 2:
                pass
        ...
        return SelfState(next_life, next_pos, next_vel, next_voltage, next_item_id,\
                ...
                next_attack_distance, next_item_used,\
                self.env)
...
def score(selfstate, env):
    score = 0
    score += selfstate.life * 1000

    # get neareast platform distance
    nearest_dist = 1000000
    for platform in env.platforms:
        d = env.helper.get_distance(selfstate.pos, (pg.Vector2(platform[0]) + pg.Vector2(platform[1])) / 2)
        nearest_dist = d if d < nearest_dist else nearest_dist
    score += 100000 / nearest_dist
    # get attack
    for d in selfstate.attack_distance:
        score += 10000 / d
    # item used
    for item_id in selfstate.item_used:
        score += ITEM_USE_SCORE[item_id]
    score += ITEM_KEEP_SCORE[selfstate.item_id]
    score -= selfstate.voltage
    score -= selfstate.uncontrollable_time
    score += selfstate.invincible_time
    score -= selfstate.attack_cd

    nearest_dist = 1000000
    for entity in env.entities:
        d = env.helper.get_distance(selfstate.pos, pg.Vector2(entity))
        nearest_dist = d if d < nearest_dist else nearest_dist
    score -= 50000 / nearest_dist
    return score